...
export default async function RangePage({ params }: PageProps) {
  ...
  // Build tag list for header chips
  const techTags = [
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main id="main-content" className="max-w-2xl mx-auto px-4 sm:px-6 pt-28 pb-20">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {techTags.map((t) => (
            <span key={t} className="text-xs font-medium px-2.5 py-1 border border-slate-200" style={{ color: '#1B5E5E', borderColor: '#cde0e0' }}>
              {t}
            </span>
          ))}
          {amenityTags.map((t) => (
        ...
        <Divider />
        <div className="py-5">
          <p className="text-sm font-medium text-slate-600 mb-0.5">Own this range?</p>
